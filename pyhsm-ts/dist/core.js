...
const self_test_js_1 = require("./self-test.js");
const shamir_js_1 = require("./shamir.js");
const SALT_LEN = 16;
const NONCE_LEN = 12;
const TAG_LEN = 16;
...
class PyHSM {
    store = { version: 3, keys: {} };
    storePath;
    masterPasswordBuf = Buffer.alloc(0);
    sessionActive = false;
    lastActivity = 0;
    sessionTimer = null;
    sessionTimeoutMs;
    ...
    constructor(config) {
        ...
        if (config.masterPassword) {
            ...
            const shares = config.shares.map((s) => JSON.parse(s));
            const reconstructed = (0, shamir_js_1.reconstructMasterPassword)(shares);
        ...
        this.audit = new audit_js_1.AuditLog(auditPath);
        this.rateLimiter = new rate_limiter_js_1.RateLimiter(parseInt(process.env.PYHSM_RATE_LIMIT || "100", 10), parseInt(process.env.PYHSM_RATE_WINDOW_MS || "60000", 10));
        this.metrics = new metrics_js_1.MetricsCollector();
        // Run self-tests before accepting any operations
        try {
            (0, self_test_js_1.runSelfTests)();
    ...
    static async create(config) {
        ...
        let preSalt = null;
        if (node_fs_1.default.existsSync(config.storePath)) {
            const raw = node_fs_1.default.readFileSync(config.storePath);
            ...
            const tempPasswordBuf = config.masterPassword
                ...
                : Buffer.from((0, shamir_js_1.reconstructMasterPassword)((config.shares || []).map(s => JSON.parse(s))), "utf8");
            preDerivedKey = await argon2_1.default.hash(tempPasswordBuf, {
        ...
        const instance = Object.create(PyHSM.prototype);
        ...
        if (config.masterPassword) {
            ...
            const shares = config.shares.map(s => JSON.parse(s));
            instance.masterPasswordBuf = Buffer.from((0, shamir_js_1.reconstructMasterPassword)(shares), "utf8");
        }
        else {
            throw new Error("PyHSM: masterPassword or shares required");
        }
        ...
        const auditPath = config.auditLogPath || config.storePath + ".audit.jsonl";
        instance.audit = new audit_js_1.AuditLog(auditPath);
        instance.rateLimiter = new rate_limiter_js_1.RateLimiter(parseInt(process.env.PYHSM_RATE_LIMIT || "100", 10), parseInt(process.env.PYHSM_RATE_WINDOW_MS || "60000", 10));
        instance.metrics = new metrics_js_1.MetricsCollector();
        (0, self_test_js_1.runSelfTests)();
        ...
        if (!instance._cachedSalt) {
            instance._cachedSalt = node_crypto_1.default.randomBytes(SALT_LEN);
    ...
    zeroize() {
        ...
        if (this._cachedDerivedKey) {
            zeroBuffer(this._cachedDerivedKey);
            this._cachedDerivedKey = null;
        }
        if (this._cachedSalt) {
            zeroBuffer(this._cachedSalt);
            this._cachedSalt = null;
        }
        if (this.callerSecret) {
            zeroBuffer(this.callerSecret);
            this.callerSecret = null;
        }
    }
    ...
    deriveKey(salt) {
        ...
        return node_crypto_1.default.pbkdf2Sync(this.masterPasswordBuf, salt, 480_000, 32, "sha256");
    }
    async deriveKeyAsync(salt) {
        const raw = await argon2_1.default.hash(this.masterPasswordBuf, {
            type: argon2_1.default.argon2id,
            salt,
            memoryCost: ARGON2_MEM_COST,
            timeCost: ARGON2_TIME_COST,
    ...
    wrapKey(kek, keyData) {
        const cipher = node_crypto_1.default.createCipheriv("aes-256-wrap-pad", kek, Buffer.alloc(4, 0xa6));
        return Buffer.concat([cipher.update(keyData), cipher.final()]);
    }
    /** AES-KWP unwrap. */
    unwrapKey(kek, wrapped) {
        const decipher = node_crypto_1.default.createDecipheriv("aes-256-wrap-pad", kek, Buffer.alloc(4, 0xa6));
        return Buffer.concat([decipher.update(wrapped), decipher.final()]);
    }
    /** Derive a wrapping key (KEK) from the master password for per-key wrapping. */
    deriveKek() {
        return node_crypto_1.default.createHmac("sha256", this.masterPasswordBuf).update("pyhsm-kek-v1").digest();
    }
    computeHmac(data, key) {
        return node_crypto_1.default.createHmac("sha256", key).update(data).digest();
    }
    load() {
        if (!node_fs_1.default.existsSync(this.storePath))
            return;
        const raw = node_fs_1.default.readFileSync(this.storePath);
        ...
        if (!node_crypto_1.default.timingSafeEqual(storedHmac, expectedHmac)) {
            zeroBuffer(key);
    ...
    save() {
        ...
        const salt = this._cachedSalt ? Buffer.from(this._cachedSalt) : node_crypto_1.default.randomBytes(SALT_LEN);
        const key = this.deriveKey(salt);
        // Update cache if we generated a new salt (PBKDF2 fallback path)
        if (!this._cachedSalt) {
            this._cachedSalt = Buffer.from(salt);
        }
        const nonce = node_crypto_1.default.randomBytes(NONCE_LEN);
        ...
        // Atomic write
        const tmp = this.storePath + ".tmp." + node_crypto_1.default.randomBytes(4).toString("hex");
        node_fs_1.default.writeFileSync(tmp, fileData, { mode: 0o600 });
        node_fs_1.default.renameSync(tmp, this.storePath);
    }
    handleTamper(reason) {
    ...
    authenticateCaller(callerId) {
        ...
        const expected = node_crypto_1.default.createHmac("sha256", this.callerSecret)
            .update(service).digest("hex");
        return constantTimeEqual(providedHmac, expected);
    }
    checkACL(entry, callerId) {
    ...
    // --- Policy Enforcement ---
    enforcePolicy(entry, operation, callerId) {
    ...
    // --- Key Operations ---
    generateKey(keyId, policy, callerId = "system") {
        this.assertSession();
        (0, types_js_1.validateKeyId)(keyId);
        if (this.store.keys[keyId])
            throw new Error(`PyHSM: key '${keyId}' already exists`);
        const rawKey = node_crypto_1.default.randomBytes(32);
    ...
    rotateKey(keyId, callerId = "system") {
        this.assertSession();
        (0, types_js_1.validateKeyId)(keyId);
        const entry = this.store.keys[keyId];
        if (!entry)
            throw new Error(`PyHSM: key '${keyId}' not found`);
    ...
    destroyKey(keyId, callerId = "system") {
        this.assertSession();
        (0, types_js_1.validateKeyId)(keyId);
        const entry = this.store.keys[keyId];
        if (!entry)
     ...
     */
    encrypt(keyId, plaintext, callerId = "system") {
        this.assertSession();
        (0, types_js_1.validateKeyId)(keyId);
        const entry = this.store.keys[keyId];
        if (!entry)
        ...
        const nonce = node_crypto_1.default.randomBytes(NONCE_LEN);
        const sivCipher = (0, aes_1.siv)(new Uint8Array(aesKey), new Uint8Array(nonce));
     ...
     */
    decrypt(keyId, ciphertextB64, callerId = "system") {
        this.assertSession();
        (0, types_js_1.validateKeyId)(keyId);
        const entry = this.store.keys[keyId];
        if (!entry)
        ...
        const sivDecipher = (0, aes_1.siv)(new Uint8Array(aesKey), nonce);
    ...
    // --- Backup ---
    createBackup(callerId = "system") {
        this.assertSession();
        const dir = this.backupDir;
        if (!dir)
            throw new Error("PyHSM: PYHSM_BACKUP_DIR not configured");
        if (!node_fs_1.default.existsSync(dir))
            node_fs_1.default.mkdirSync(dir, { recursive: true, mode: 0o700 });
        const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        const backupPath = `${dir}/pyhsm-backup-${timestamp}.enc`;
        node_fs_1.default.copyFileSync(this.storePath, backupPath);

...
hsm/
  ...
  storage.py        — KeyStore: Argon2id (primary) / PBKDF2 (fallback) key derivation,
...
tests/
  test_pyhsm.py     — 112 pytest tests
```
...
Both layers now use Argon2id as the primary key derivation function (OWASP recommended, 64 MB memory-hard). The Python layer falls back to PBKDF2-SHA256 at 480,000 iterations only if `argon2-cffi` is unavailable.
